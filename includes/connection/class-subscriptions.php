 ...
 *
 * Registers connections to Subscriptions
 *
 * @package WPGraphQL\WooCommerce_Subscription\Connection
 * @since   0.0.2
 */

namespace WPGraphQL\WooCommerce_Subscriptions\Connection;
...
class Subscriptions {
    ...
    /**
     * Registers the various connections from other Types to Subscriptions
     */
    public static function register_connections() {
        // From RootQuery To Subscriptions.
        register_graphql_connection( self::get_connection_config() );

        // From Customer To Subscriptions.
            ...
            self::get_connection_config(
                array(
                    ...
                    'resolve'       => function( $source, array $args, AppContext $context, ResolveInfo $info ) {
                        ...
                        return self::get_customer_subscription_connection( $resolver, $source );
                    },
                )
            )
        );

        // From Order To Subscriptions.
        register_graphql_connection(
            self::get_connection_config(
                array(
                    'fromType'       => 'Order',
        ...
        // From Subscription To Renewal Orders.
        register_graphql_connection(
            self::get_connection_config(
                array(
     ...
     * @return array
     */
    public static function get_connection_config( $args = array(), $post_type = 'shop_subscription' ): array {
        ...
        return array_merge(
            array(
                ...
                'resolve'        => function( $source, array $args, AppContext $context, ResolveInfo $info ) use ( $post_object ) {
                    ...
                    $args = $not_manager && 'shop_subscription' === $post_object->name
                        ? \array_intersect_key( $args, array_keys( array() ) ) // @todo
                        : $args;
                    ...
                    /**
                     * If not shop manager, restrict results to subscriptions owned by querying user
                     * and return the connection.
                     */
                    if ( 'shop_subscription' === $post_object->name ) {
                        $empty_results = array(
                            'pageInfo' => null,
                            'nodes'    => array(),
                            'edges'    => array(),
                    ...
                    return $not_manager
                        ? self::get_customer_subscription_connection( $resolver, \WC()->customer )
                        : $resolver->get_connection();
                },
            ),
            $args
    ...
    /**
     * Returns subscription connection filtered by customer.
     *
     ...
     * @return array
     */
    private static function get_customer_subscription_connection( $resolver, $customer ) {
        $customer_id = $customer->get_id();

        // If not "ID" set bail early by returning an empty connection.
        if ( empty( $customer_id ) ) {
            return array();
        }

        $resolver->set_query_arg( 'meta_key', '_customer_user' );
        $resolver->set_query_arg( 'meta_value', $customer_id );
    ...
    public static function post_object_connection_query_args( $query_args, $source, $args, $context, $info ) {
        ...
        if ( empty( $args['where'] ) || empty( $args['where']['statuses'] ) ) {
            // Subscription shares status with order
    ...
    public static function map_input_fields_to_wp_query( $query_args, $where_args, $source, $args, $context, $info, $post_type ) {
        ...
        $query_args = apply_filters(
            'graphql_map_input_fields_to_subscription_query',
            $query_args,
            $where_args,
            $source,
            $args,
